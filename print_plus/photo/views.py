...
def ajax_doctype(request):
    ...
    if data["color"] == "colored":
        ...
        res = {"content": list(set(type_list))}
    return JsonResponse(res)
...
def ajax_order(request):
    ...
    try:
        ...
        doc_type = (
            "С картинками" if data["doc_type"] == "С_картинками" else data["doc_type"]
        )
        orders = Documents.objects.filter(
            type=color, format=data["format"], document=doc_type
        )
        for i in orders:
            price = int(i.price)
        total_price = price * pages * ex
        res = {"error": "success", "price": total_price}
        return JsonResponse(res)
    except:
        res = {
